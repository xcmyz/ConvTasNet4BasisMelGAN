...
class ConvTasNet(nn.Module):
    ...
    def forward(self, mixture):
        ...
        est_source, source_w = self.decoder(mixture_w, est_mask)

        # T changed after conv1d in encoder, fix it here
        T_origin = mixture.size(-1)
        T_conv = est_source.size(-1)
        est_source = F.pad(est_source, (0, T_origin - T_conv))
        return est_source, mixture_w, source_w

    def test(self, mixture):
        """
        Args:
            mixture: [M, T], M is batch size, T is #samples
        Returns:
        ...
        mixture_w = self.encoder(mixture)
        est_mask = self.separator(mixture_w)
        est_source, _ = self.decoder(mixture_w, est_mask)

        # T changed after conv1d in encoder, fix it here
        T_origin = mixture.size(-1)
        T_conv = est_source.size(-1)
        est_source = F.pad(est_source, (0, T_origin - T_conv))
        return est_source, mixture_w

    def autoencode(self, wav):
        weight = self.encoder(wav)
        est_mask = self.separator(weight)
        est_source, weight_ = self.decoder.autoencode(weight, est_mask)
...
class Decoder(nn.Module):
    ...
    def forward(self, mixture_w, est_mask):
        ...
        return est_source, source_w

    def autoencode(self, weight, est_mask):
        weight_ = torch.unsqueeze(weight, 1) * est_mask
        weight_ = torch.transpose(weight_, 2, 3)
        est_source = self.basis_signals(weight_)
        est_source = overlap_and_add(est_source, self.L // 2)
        return est_source, weight_

    def vocoder(self, weight):
        est_source = self.basis_signals(weight)
        est_source = overlap_and_add(est_source, self.L // 2)
        return est_source
    ...
    def __init__(self, N, B, H, P, X, R, C, norm_type="gLN", causal=False,
        ...
        for r in range(R):
            ...
            for x in range(X):
                dilation = 2**x
                padding = (P - 1) * \
                    dilation if causal else (P - 1) * dilation // 2
